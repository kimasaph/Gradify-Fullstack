...
export const getReportById = async (reportId, authHeader) => {
    const response = await axios.get(`${API_BASE_URL}/${reportId}`, { headers: authHeader });
    return response.data;
};

export const getReportsByStudentId = async (studentId, authHeader) => {
    const response = await axios.get(`${API_BASE_URL}/student/${studentId}`, { headers: authHeader });
    return response.data;
};

export const getReportsByTeacherId = async (teacherId, authHeader) => {
    const response = await axios.get(`${API_BASE_URL}/teacher/${teacherId}`, { headers: authHeader });
    return response.data;
};

export const getReportsByClassId = async (classId, authHeader) => {
    const response = await axios.get(`${API_BASE_URL}/class/${classId}`, { headers: authHeader });
    return response.data;
};

export const updateReport = async (reportId, reportDTO, authHeader) => {
    const response = await axios.put(`${API_BASE_URL}/${reportId}`, reportDTO, { headers: authHeader });
    return response.data;
};
